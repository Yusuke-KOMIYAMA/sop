    ...
    'tablet.controller.Main', {
        ...
        onLoginSucceed: function(params) {
            ...
            Ext.ComponentQuery.query('pj-list > titlebar')[0].setTitle(
                'プロジェクト一覧&nbsp;&nbsp;&nbsp;<span style="font-size:80%;">' +
                params.grp_name +
            ...
            Ext.Ajax.request({
                ...
                success: function(res, eopts) {
                    if (Ext.decode(res.responseText).total > 0) {
                        Ext.Msg.show({
                            title: '通知',
                            message: '作業完了（チェック待ち）のテンプレートがあります。',
                            scope: this,
                            buttons: [{
                                itemId: 'yes',
                                text: '作業完了（チェック待ち）一覧へ'
                            }, {
                                itemId: 'no',
                                text: 'スキップ'
                            }],
        ...
        onLoadPjStore: function(store, recs, success, ope, eopts) {
            ...
            this.getPjList().query('toolbar')[0].setTitle(response.total + '件のアイテム');
        },
        ...
        onLoadProvisionalfixStore: function(store, recs, success, ope, eopts) {
            var response = Ext.decode(ope.getResponse().responseText);
            this.getProvisionalfixList().query('toolbar')[0].setTitle(response.total + '件のアイテム');
        },
        ...
        onLoadPjSmplStore: function(store, recs, success, ope, eopts) {
            var response = Ext.decode(ope.getResponse().responseText);
            this.getPjSmplList().query('toolbar')[0].setTitle(response.total + '件のアイテム');
        },
        ...
        onLoadSopStore: function(store, recs, success, ope, eopts) {
            var response = Ext.decode(ope.getResponse().responseText);
            this.getSopList().query('toolbar')[0].setTitle(response.total + '件のアイテム');
        },
        ...
        onTapSop: function(list, idx, trgt, rec, e, eopts) {
            if (rec.data.file_status == 0) // 未入力
            {
                ...
                {
                    Ext.Msg.alert('Notice', 'このアイテムは入力ファイルを準備中です');
                    return;
                }
        ...
        onTapFilePanelBackBtn: function(btn, e, eopts) {
            ...
            if (div != 'fix') {
                var that = this;
                Ext.Msg.confirm('Confirm', '標準作業手順書一覧に戻ります。よろしいですか？', submit);
        ...
        onTapFilePanelFixBtn: function(btn, e, eopts) {
            if (this.getFilePanel().query('#hdn_div')[0].getValue() == 'add') {
                Ext.Msg.alert('Notice', '一旦保存してください');
                return;
            }
        ...
        showLoginPanel: function(args) {
            if (args.from_fn == 'launch') {
                ...
                var that = this;
                Ext.Msg.confirm('Confirm', 'ログアウトします。よろしいですか？', submit);
        ...
        showBackMainPanel: function(args) {
            var that = this;
            Ext.Msg.confirm('Confirm', 'ホームに戻ります。よろしいですか？', submit);
        ...
        showProvisionalfixList: function(args) {
            ...
            this.getProvisionalfixListTbar().setTitle('作業完了（チェック待ち）標準作業手順書一覧');
        ...
        showPjSmplList: function(args) {
            // tbar タイトル、戻るボタン テキスト設定
            this.getPjSmplListTbar().setTitle(args.pj_name + '&nbsp;&nbsp;&nbsp;対象サンプル一覧');
        ...
        showSopList: function(args) {
            ...
            // tbar タイトル、戻るボタン テキスト設定
            this.getSopListTbar().setTitle(args.smpl_given_no + '&nbsp;&nbsp;&nbsp;標準作業手順書一覧');
        ...
        showProvisionalFixFilePanel: function(args, fix_user, refixable) {
            ...
            Ext.Msg.alert('Notice', 'このアイテムは入力が完了しています', submit);
        ...
        showConfirmFixFilePanel: function(args) {
            ...
            Ext.Ajax.request({
                ...
                success: function(res, eopts) {
                    ...
                    // tbar タイトル、戻るボタン テキスト設定
                    filepanel.down('titlebar').setTitle(retprms.sop_name);
                    filepanel.down('titlebar').down('#back_btn').setText('<span style="font-size:80%;">作業完了（チェック待ち）一覧</span>');
            ...
            if (params != null) {
                ...
                Ext.Ajax.request({
                    ...
                    failure: function(res, eopts) {
                        ...
                        var error_notification = filepanel.down('#error_notification');
                        error_notification.setHtml('サーバにデータを保存できませんでした。<br />ネットワークに繋がっているかどうか確認してください。');
