...
if (! $grp) {
    ...
    \Sop\Api::exitWithError(array("グループが見つかりません"));
}
$grp_id = $grp['grp_id'];
...
if($div == 'add')
{
    if($cnt > 0)
    {
        ...
        \Sop\Api::exitWithError(array("このユーザーIDは既に使用されています"));
    }
}
...
//{
    ...
    {
        \Sop\Log::warning(__FILE__, __LINE__, 'Invalid group is specified.');
        \Sop\Api::exitWithError(array("このグループは既に削除されています: {$grp_id}"));
    }
//}
...
if (\Sop\Session::getSiteData('user_id') == $user_id &&
    ...
    \Sop\Api::exitWithError(array("自分自身の管理権限は外せません。"));
}
...
if($div == 'add')
{
    ...
    if($deleted_cnt > 0){
    	...
    	if(!$rslt)
    	{
        	...
        	\Sop\Api::exitWithError(array("登録に失敗しました: user"));
    	}
...
//{
    ...
    if(!$rslt)
    {
        ...
        \Sop\Api::exitWithError(array("登録に失敗しました: grp_id"));
    }
//}
