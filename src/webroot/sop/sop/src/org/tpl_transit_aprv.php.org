...
foreach (R::getAll($sql, array(':tpl_id' => $tpl_id)) as $schema) {
    ...
    if ($schema['schema_type'] == $SCHEMA_TYPE_SRC) {
        ...
        if (count(R::getAll($sql, array(':tpl_id' => $tpl_id))) == 0) {
            ...
            \Sop\Api::exitWithError(array("入力フォームを追加してから申請してください。"));
        }
    }
}
...
if (!$rslt) {
    ...
    \Sop\Api::exitWithError(array("登録に失敗しました: transit_aprv"));
}
...
echo json_encode(
    array('success'=>true,
          'msg'=> \Sop\Api::htmlEncodeLines(array($new_aprv_flg == 0 ? '承認可能にしました' : '承認申請を取り消しました')),
